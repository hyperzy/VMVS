...
private:
    ...
    */
     ...
     * @param limits Boundary(Surface) coordinates array
     * @param bound_coord Bounding box vertex coordinates
     */
    void Determine_bound_coord();

public:
    std::vector<std::vector<std::vector<dtype>>> grid;
    explicit Grid(const std::vector<Camera> &all_cams);

    void Init_grid();

    std::vector<dtype> Get_limits() const;
